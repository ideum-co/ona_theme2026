...
- **Extend media to screen edge:** checkbox. Show it only when Section width is Page.
...
- **Extend media to screen edge** lets the media column cross the page-width/max-width boundary to the adjacent viewport edge. Text and wordmark remain inside the constrained inner layout.
...
The screen-edge extension applies only on desktop, where the section has columns. On mobile, media and content remain within the established single-column flow.

## Verification

Focused tests will verify:

- all six schema controls, their options, defaults, ranges, and visibility conditions;
...
- extending media does not move text outside the constrained wrapper;
